...
(function () {
    ...
    const THEMES = ["sabah", "gunduz", "alaca"];
    const THEME_LABELS = { sabah: "Sabah Mavisi", gunduz: "Gündüz", alaca: "Alaca" };
    ...
    async function boot() {
        ...
        engine.onProgress = (done, total) => {
            if (loaderText) loaderText.textContent = `Sabah ediyor… ${done}/${total}`;
        };
        ...
        requestAnimationFrame(() => {
            ...
            if (!Storage.getFirstSeen()) {
                ...
                window.LL?.emit('cover_gate_opened', { bookId: 'fabl' });
            }
        });
    }
    ...
    function handleSpreadChange(spreadIdx) {
        ...
        window.LL?.emit('page_turn', { bookId: 'fabl', spread: spreadIdx });
    ...
    function wireUI() {
        ...
        dom.btnBookmark.addEventListener("click", () => {
            ...
            if (added) window.LL?.emit('bookmark_add', { bookId: 'fabl', spread: idx });
        ...
        dom.btnTheme.addEventListener("click", () => {
            ...
            window.LL?.emit('theme_change', { bookId: 'fabl', theme: next });
        ...
        dom.btnFont.addEventListener("click", async () => {
            ...
            window.LL?.emit('font_scale_change', { bookId: 'fabl', typeStep: next });
        ...
        dom.btnPerf.addEventListener("click", () => {
            ...
            try { localStorage.setItem("fabl:v1:perfMode", JSON.stringify(next)); } catch (e) { /* */ }
    ...
    function applyTheme(name) {
        if (!THEMES.includes(name)) name = "sabah";
